...
@section('title', 'Create Transaksi Peminjaman Buku')
                ...
                <table class="table table-striped table-bordered" id="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <button class="btn btn-sm btn-primary" id="pilih" data-id="{{ $data->id }}"
                                    data-isbn="{{ $data->no_isbn }}" data-judul="{{ $data->judul_buku }}"
...
<script>
    $(document).ready(function() {
            ...
            // Validasi denda sebelum submit
            $('#forms').submit(function(e) {
                const anggotaId = $('select[name="id_anggota_peminjaman"]').val();
                if (!anggotaId) {
                    e.preventDefault();
                ...
                // Cek apakah ada item di temporary dengan cara yang lebih sederhana
                const tableRows = $('#table1 tr');
                let hasItems = false;
                
                // Cek apakah ada baris dengan data (bukan baris kosong atau "Tidak ada buku yang dipilih")
                tableRows.each(function() {
                    const rowText = $(this).text().trim();
                    if (rowText !== '' && rowText !== 'Tidak ada buku yang dipilih') {
                        // Cek apakah baris ini memiliki nomor urut (kolom pertama)
                        const firstCell = $(this).find('td:first').text().trim();
                        if (firstCell !== '' && !isNaN(firstCell) && parseInt(firstCell) > 0) {
                            hasItems = true;
                ...
                if (!hasItems) {
                    e.preventDefault();
                    Swal.fire('Error', 'Tidak ada buku yang dipilih. Silakan tambahkan buku terlebih dahulu.', 'error');
                    return false;
                }
                
                // Konfirmasi sebelum submit
                if (!confirm('Apakah Anda yakin ingin menyimpan peminjaman ini?')) {
                    e.preventDefault();
                    return false;
                }
            });
        ...
        function load() {
            $('#table1').load('panggil-temp', function() {
                // Cek apakah ada item setelah load
                const tableRows = $('#table1 tr');
                let hasItems = false;
                ...
                // Tampilkan/sembunyikan tombol Simpan berdasarkan ada tidaknya item
                if (hasItems) {
                    $('#proses').show();
                } else {
                    $('#proses').hide();
        ...
        function simpanTemp() {
            ...
            // Validasi input
            if (isbn == '') {
                Swal.fire('Error', 'ISBN Buku harus diisi', 'error');
                return;
            }
            ...
            if (jumlah == '' || jumlah <= 0) {
                Swal.fire('Error', 'Jumlah Buku harus diisi dan lebih dari 0', 'error');
                return;
            }

            if (jumlah > jumlah_buku) {
                Swal.fire('Error', 'Stock Buku Tidak Mencukupi', 'error');
                return;
            }
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success == 200) {
                        ...
                        load()
                        $('#proses').show();
                        Swal.fire('Success', 'Buku berhasil ditambahkan ke keranjang', 'success');
