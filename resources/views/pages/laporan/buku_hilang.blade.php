...
<head>
    ...
    <title>{{ $title ?? 'Laporan Buku Hilang' }}</title>
    <style>
        .table thead th {
...
<body>
    @php $title = 'Laporan Buku Hilang'; @endphp
    ...
    <div class="container">
        <table class="table table-bordered mb-4 table-striped">
            ...
            <tbody>
                @forelse ($bukuHilang as $no => $data)
                <tr>
                    <td>{{ $bukuHilang->firstItem() + $no }}</td>
        ...
        {{ $bukuHilang->withQueryString()->links() }}
    </div>
    <script>
        window.print();
    </script>
